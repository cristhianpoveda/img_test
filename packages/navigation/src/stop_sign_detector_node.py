...
class StopSignDetector(DTROS):
    ...
    def srv_stop(self, req=None):
        ...
        rospy.loginfo(self.image_np.shape)
        rate = rospy.Rate(self._calculation_rate)
        
        while 1:
        
            # remove the image undesired section
            #height, width = undistorted_img.shape[0:2]
            #image_stop = undistorted_img[int(height*3/8) :, int(width/4):int(width*0.75), :]
            ...
            if len(contours) != 0:
                ...
                #cv2.drawContours(undistorted_img, contours, -1, (0,255,0), 2)
