...
class SimpleProblem{
    ...
    public:
        ...
        float joint_sigmoid_gaussian(float x1, float x2){
    
            // header
            float th1 = 0.2, th2 = 0.05;
    
            float gaussian = abs(normal_dist(x2) - normal_dist(x1));
    
            // probability of being empty
            float p = 1 - 1 / (1 + exp(-2 * (gaussian - th1) / th2));
        ...
        void joint_probability_distribution(){
            ...
            if (o.user == 0 && o.point.projection_distance <= projection_lim){
                ...
                if(o.trajectory_group == 0 || o.trajectory_group == destination){
                    ...
                    row = joint_sigmoid_gaussian(x1, x2);
                }
                ...
                row = o.ANN_value * joint_sigmoid_gaussian(x1, x2);
        ...
        bool srv_callback(std_srvs::EmptyRequest &request,
                          std_srvs::EmptyRequest &response){
            ...
            front.push_back(frontal1);
            front.push_back(frontal2);
...
int main(int argc, char** argv)
{
  ...
  ros::ServiceServer decision_srv = nh.advertiseService("/decision_srv", &SimpleProblem::srv_callback, &sp);
